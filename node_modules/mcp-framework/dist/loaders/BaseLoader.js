...
export class BaseLoader {
    ...
    resolveDirectory(subdirectory, basePath) {
        if (basePath) {
    ...
    async hasItems() {
        try {
            ...
            logger.debug(`No ${this.config.subdirectory} directory found: ${error.message}`);
            return false;
        }
    }
    async loadItems() {
        try {
            ...
            for (const file of files) {
                try {
                    ...
                    logger.debug(`Attempting to load ${this.config.subdirectory.slice(0, -1)} from: ${fullPath}`);
                    ...
                    logger.error(`Error loading ${this.config.subdirectory.slice(0, -1)} ${file}: ${error.message}`);
                }
            }
            logger.debug(`Successfully loaded ${items.length} ${this.config.subdirectory}: ${items.map(this.getItemName).join(', ')}`);
            return items;
        }
        catch (error) {
            logger.error(`Failed to load ${this.config.subdirectory}: ${error.message}`);
            return [];
        }
    }
}

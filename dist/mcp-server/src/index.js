...
export const logger = {
    ...
    error: (...args) => console.error('[MCP Server ERROR]', ...args),
    warn: (...args) => console.warn('[MCP Server WARN]', ...args),
    debug: (...args) => process.env.NODE_ENV !== 'production' ? console.debug('[MCP Server DEBUG]', ...args) : null,
};
...
async function startServer() {
    try {
        ...
        await server.start();
        if (isShuttingDown)
            return; // Don't continue if shutdown started
        ...
        const httpServer = server.transport?.server?.httpServer;
        if (httpServer) {
            ...
            if (app && typeof app.use === 'function') {
                ...
                if (isShuttingDown)
                    return; // Don't log if shutdown started
                logger.info(`✅ x402 routes integrated into MCP server`);
            }
            else {
                if (!isShuttingDown)
                    logger.warn('Could not access Express app to add x402 routes');
            }
        }
        else {
            if (!isShuttingDown)
                logger.warn('Could not access HTTP server to add x402 routes');
        }
        if (!isShuttingDown) {

...
const toolLogger = {
    info: (...args) => console.log('[TipUserX402Tool]', ...args),
    error: (...args) => console.error('[TipUserX402Tool ERROR]', ...args),
    warn: (...args) => console.warn('[TipUserX402Tool WARN]', ...args),
    debug: (...args) => console.debug('[TipUserX402Tool DEBUG]', ...args)
};
export default class TipUserX402Tool extends MCPTool {
    name = "tip_user_x402";
    ...
    async execute(input) {
        try {
            ...
            const x402Client = withPaymentInterceptor(axios.create({ baseURL, timeout: 30000 }), account);
    ...
    async getWalletAddress(tipMdUserId) {
        try {

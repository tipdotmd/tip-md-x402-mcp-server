...
const toolLogger = {
  info: (...args: any[]) => console.log('[GetUserWalletTypesTool]', ...args),
  error: (...args: any[]) => console.error('[GetUserWalletTypesTool ERROR]', ...args),
  warn: (...args: any[]) => console.warn('[GetUserWalletTypesTool WARN]', ...args),
  debug: (...args: any[]) => console.debug('[GetUserWalletTypesTool DEBUG]', ...args)
};
...
// Type for the input parameters, inferred from the Zod schema
export type GetUserWalletTypesParams = z.infer<typeof UserWalletTypesParamsZodSchema>;

// Define response types for different scenarios
 ...
 */
const extractWalletTypes = (user: User): string[] => {
  const walletTypes: string[] = [];
  if (user.solanaAddress && user.solanaAddress.trim() !== '') { walletTypes.push('solana'); }
  if (user.ethereumAddress && user.ethereumAddress.trim() !== '') { walletTypes.push('ethereum'); }
  if (user.bitcoinLightningAddress && user.bitcoinLightningAddress.trim() !== '') { walletTypes.push('bitcoinLightning'); }
  if (user.wallets && Object.keys(user.wallets).length > 0) {
    Object.keys(user.wallets).forEach(type => {
      if (!walletTypes.includes(type)) { walletTypes.push(type); }
    });
  }
  return walletTypes;
};
...
export default class GetuserwallettypesTool extends MCPTool<GetUserWalletTypesParams> {
  ...
  async execute(input: GetUserWalletTypesParams): Promise<UserWalletTypesResponse> {
